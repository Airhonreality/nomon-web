...
async function main() {
	...
	const recibidos =
		await sql`SELECT COUNT(*)::int AS n FROM "Mensaje" WHERE direccion = 'RECIBIDO'`;
	const smoke =
		await sql`SELECT id, de, para, asunto, direccion, created_at FROM "Mensaje" WHERE message_id = 'smoke-test-001'`;
