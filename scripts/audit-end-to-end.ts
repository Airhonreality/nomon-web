...
const SECRET =
	"a6702500c40b1cba49c8a6d44d966829184925c5fd9a763e8d5a7e19d6bd3afe";
...
const listRes = await fetch(`${BASE}/api/correo`, {
...
for (const m of bandeja.slice(0, 5)) {
	console.log(
		`   - [${m.direccion}] de=${m.de} para=${m.para} asunto="${m.asunto}" adjuntos=${(m.adjuntos || []).length}`,
	);
}
...
console.log(
	"\n3) POST /api/webhooks/incoming-email ->",
	webhookRes.status,
	await webhookRes.text(),
);

// 4. Re-listar para confirmar que el mensaje entró
const listRes2 = await fetch(`${BASE}/api/correo`, {
	headers: { Cookie: cookie },
});
...
console.log(
	"   Ultimo:",
	bandeja2[0]
		? `${bandeja2[0].direccion} de ${bandeja2[0].de} - ${bandeja2[0].asunto}`
		: "vacia",
);
